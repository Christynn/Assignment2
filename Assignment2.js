...
function generateItemHTML(product, clickable = false) {
  ...
  if (clickable) {$(productItem).on("click", addProduct);  }
  return productItem;
}
...
function addProduct() {
  ...
  displayCart();
  $("#total").text("$" + totalPrice.reduce(function(a, b) {
    return a + b;
  }, 0));
  showPrice();
 }
 ...
 function removeProductFromCart() {
  ...
  showPrice();
  product.on("click", removeProductFromCart);
 }
...
function showPrice() {
  let total = SHOPPING_CART.reduce(function(acc, productKey) {
